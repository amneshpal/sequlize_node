...
// row query ------->   get user by row query 
...
const getUser = async (req, res) => {
    ...
    try {
        ...
        // replacement  ex id ya name se kuchh find karna  h to iska use karte h  
            // const result = await sequelize.query('SELECT *FROM user WHERE id = ? and firstname = ?', {
            // replacements:['1','abhinesh'],
            // type: QueryTypes.SELECT,

            // row query se getter or setter or virtual method (fullname ) nahi show hoga iske liye hm model ko map karte h
            model: UserModel,
            mapToModel: true,

            // plain: true   // ise use karne pe fisrt data milta h 
        })
...
const restoresoftDelete = async (req, res) => {
    try {
        const user = await UserModel.findByPk(req.params.id);
        if (!user || user.isDeleted === 0) return res.status(404).json({
            message: "User not found"
        });
